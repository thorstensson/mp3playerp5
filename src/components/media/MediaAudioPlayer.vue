...
const doScrub = (e: MouseEvent) => {
  if (progressBar.value && audioEl.value) {
    const scrubTime =
      (e.offsetX / progressBar.value!.offsetWidth) * audioEl.value.duration
...
const progressUpdate = () => {
  ...
  progressBarFilled.value!.style.flexBasis = `${roundedPercent}%`
}
...
onMounted(() => {
  ...
  if (audioEl.value && progressBar.value) {
    ...
    progressBar.value.addEventListener(
      "mousemove",
      (e) => mousedown && doScrub(e)
    )
...
<template>
  <div class="player-wrapper">
    ...
    <div class="panel">
      <div class="panel__box">
        <div
          :class="{ 'panel__box__track--paused': !isPlaying }"
          class="panel__box__track"
          ref="panel-track"
        >
          {{ currentTrack }}
        </div>
  ...
  <MediaPlaylist
    :playlist="playlist"
    :current-track-index="trackIndex"
    @select-track="selectTrackFromPlaylist"
  />
</template>
